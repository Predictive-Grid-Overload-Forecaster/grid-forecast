...
const Help = () => {
    ...
    const [showpart, setshowpart] = useState(help_data.map(() => false));

    const togglehelp = (index) => {
        const newState = [...showpart];
        newState[index] = !newState[index];
        setshowpart(newState)
    }
                ...
                <div className="quick-guide-items">
                    {help_data.map((data, i) => (
                        <div key={i} className="guide-item">
                            ...
                            <button onClick={() => {togglehelp(i)}} className="guide-btn">Learn more</button>
                            {showpart[i] && (
                                <div>
                                    <p>{data.text}</p>
                                </div>
